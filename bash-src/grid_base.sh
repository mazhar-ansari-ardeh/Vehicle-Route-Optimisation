#!/usr/bin/env bash
# This is the original grid library that I wrote. The code in this file was refactored later into the file 'gridlib.sh'
# This file is deprecated and is kept for historical reasons.
...
function ecj_experiment()
{
    L_EXPERIMENT_NAME=$1
    L_DATASET_FILE="$2/$3.dat"
    L_NUM_VEHICLES=$4
    echo "Running experiment: $L_EXPERIMENT_NAME"
    echo "Experiment arguments: ${@}"
    L_EXPERIMENT_DIR="./$L_EXPERIMENT_NAME/$SGE_TASK_ID"
    java -cp .:tl.jar ec.Evolve     -file carp_param_base.param \
                                    -p stat.file="\$$L_EXPERIMENT_DIR/job.0.out.stat" \
                                    -p stat.gen-pop-file="$L_EXPERIMENT_DIR/population.gen" \
                                    -p eval.problem.eval-model.instances.0.file=$L_DATASET_FILE \
                                    -p generations=$GENERATIONS \
                                    -p eval.problem.eval-model.instances.0.vehicles=$L_NUM_VEHICLES \
                                    -p seed.0=$SGE_TASK_ID \
                                    -p "${@:5}"

    echo "Finished running experiment on target $L_EXPERIMENT_NAME"

    echo "Running tests on target problem $L_EXPERIMENT_NAME"
    java -cp .:tl.jar gphhucarp.gp.GPTest -file carp_param_base.param \
                                        ...
                                        -p generations=$GENERATIONS \
                                        -p seed.0=$TEST_SEED
    if [[ -z $NUM_VEHICLES_TARGET ]] && [[ -z $DATASET_TARGET ]]
    then
        SAVE_TO=/vol/grid-solar/sgeusers/mazhar/$DATASET_SOURCE.vs$NUM_VEHICLES_SOURCE:gen_$GENERATIONS/$L_EXPERIMENT_NAME
    else
    ...
    printf "Finished running tests on target $L_EXPERIMENT_NAME\n\n\n"
}
...
java -cp .:tl.jar tl.knowledge.extraction.ExtractPPT carp_param_base.param $KNOWLEDGE_SOURCE_DIR/ $KNOWLEDGE_SOURCE_DIR/pipe_tree.ppt \
                        ...
                        extract-ppt.from-generation=$3 \
                        extract-ppt.to-generation=$4 \
...
do_knowledge_experiment PPTLearning:percent_$1:niche_$2:gens_$3_$4:lr_$5 \
...
evaluate_on_test {49..0}

# PPTEvolutionState 0.8 0.1 2
#
# PPTEvolutionState 0.2 0.1 2
#
# PPTEvolutionState 0.5 0.1 2
