...
if [ -z "$DATASET_CATEGORY_SOURCE" ]; then
...
#       2.2. corrphenotypic
#       2.3. hamming
# 3. Niche radius for the clearing of the interim populaiton
# 4. Niche capacity for the clearing of the interim populaiton
# 5. Interim magnitude, that is, the number of times the interim population is larger than the actual populaiton.
# 6. The size of the decision making situations used for similarity measurements
function RandPoolFullTree() {
  local L_EXP_NAME="RandPoolFullTree:tp_$1:metric_$2:interimrad_$3:interimcap_$4:interimmag_$5:dms_${6}"
  local L_EXPERIMENT_DIR="$L_EXP_NAME/$SGE_TASK_ID"
  do_knowledge_experiment "$L_EXP_NAME" \
    -p state=tl.gphhucarp.dms.DMSSavingGPHHState \
    -p gp.tc.0.init=tl.knowledge.surrogate.SurEvalBuilder \
