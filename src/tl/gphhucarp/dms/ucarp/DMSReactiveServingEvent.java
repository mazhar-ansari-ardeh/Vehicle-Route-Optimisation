...
import gphhucarp.decisionprocess.reactive.event.ReactiveRefillThenServeEvent;
...
public class DMSReactiveServingEvent extends DMSDecisionProcessEvent {
	private NodeSeqRoute route;
	private Arc nextTask;

	public DMSReactiveServingEvent(double time,
	...
	public void trigger(DecisionProcess decisionProcess) {
		...
		if (currNode == nextTask.getFrom()) {
			...
			if (remainingDemand > remainingCapacity) {
				...
				decisionProcess.getEventQueue().add(
						new gphhucarp.decisionprocess.reactive.event.ReactiveServingEvent(route.getCost(), route, nextTask));
			}
		}
