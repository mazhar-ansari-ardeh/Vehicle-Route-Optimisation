...
import gphhucarp.gp.EnsembleSurrogatedGPHHState;
import tl.TLLogger;
...
// TODO: Maybe this class can be merged with SurrogateBuilder.
public class EnsembleSurrogateBuilder  extends HalfBuilder implements TLLogger<GPNode>
	{
		/**
...
//    public static final String P_ALLOW_DUPLICATES = "allow-duplicates";
		...
		public void setup(EvolutionState state, Parameter base)
		{
			...
			EnsembleSurrogatedGPHHState estate = (EnsembleSurrogatedGPHHState) state;
			try
			{
				Individual[] inds = PopulationUtils.loadPopulation(kbFile).subpops[0].individuals;
				estate.surFitness.setSituations(estate.getInitialSituations().subList(0,
						Math.min(estate.getInitialSituations().size(), estate.DMS_SIZE)));
				if(initSurrogatePool)
				{
					// TODO: Get pool filter here.
					estate.surFitness.updateSurrogatePool(inds, "s:gen_49");
				}
				pop.addAll(estate.surFitness.getSurrogatePool(0));
									...
									final int requestedSize)
		{
			...
			if(!pop.isEmpty() && cfCounter < populationSize * transferPercent)
			{
				...
				cfCounter++;
				log(state, knowledgeSuccessLogID, cfCounter + ": \t" + root.
						makeCTree(false, true, true) + "\n\n");
				root.parent = parent;
				return root;
			}
