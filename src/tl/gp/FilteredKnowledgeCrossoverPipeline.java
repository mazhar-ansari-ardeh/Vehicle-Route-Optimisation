...
public class FilteredKnowledgeCrossoverPipeline extends GPBreedingPipeline
{
	...
	public FilteredKnowledgeCrossoverPipeline() { parents = new GPIndividual[2]; }
	...
	public Object clone()
	{
		FilteredKnowledgeCrossoverPipeline c = (FilteredKnowledgeCrossoverPipeline)(super.clone());
	...
	//  private void getExtractor(final EvolutionState state)
	//  {
		...
		for(int q=start;q<n+start; /* no increment */)  // keep on going until we're filled up
		{
			...
			else
			{
				...
				if (parents[0].trees[t1].constraints(initializer)
					...
					state.output.fatal("GP Crossover Pipeline's two tree choices are both specified"
									   + " by the user -- but their GPTreeConstraints are not the "
									   + "same");
			}
			...
			FilteredFittedCodeFragmentBuilder builder = new FilteredFittedCodeFragmentBuilder();
			...
			for(int x=0;x<numTries;x++)
			{
				...
				e2 = (GPNode) cf2.getItem();
				rese = verifyPoints(initializer, e1, p1);
				if(!rese)
					e1 = null;
				rese = verifyPoints(initializer, e2, p2);
				if(!rese)
					e2 = null;
			...
			GPIndividual gindold, int logID)
	{
		...
		if(gindnew.fitness instanceof KozaFitness)
			fitnessnew = ((KozaFitness)fitnew).standardizedFitness();
		String treenew = gindnew.trees[0].child.makeGraphvizTree().replaceAll("\n", "") + ", ";

		Fitness fitold = gindold.fitness;
