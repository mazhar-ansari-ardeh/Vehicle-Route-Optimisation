 ...
 *
 * This class was used in the APTLGPSeondBestAux experiment but it is buggy and its implementation is wrong.
 *
 * @deprecated
 */
@Deprecated
public class UniqueFullTreeBuilderEx extends HalfBuilder implements TLLogger<GPNode>
{
                                ...
                                int argposition, int requestedSize)
    {
        ...
        if(!goodInds.isEmpty())
        {
            GPIndividual ind = (GPIndividual) goodInds.remove(0);
            root = GPIndividualUtils.stripRoots(ind).get(0);
            log(state, knowledgeSuccessLogID, "transfer," + root.makeLispTree() + "\n");
        }
        else
        {
