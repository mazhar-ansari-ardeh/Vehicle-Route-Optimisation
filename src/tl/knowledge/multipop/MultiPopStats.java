...
public class MultiPopStats extends Statistics implements SteadyStateStatisticsForm //, ec.eval.ProvidesBestSoFar
{
    ...
    /** The Statistics' log */
    public int[] statisticslog;  // stdout
    ...
    public void setup(final EvolutionState state, final Parameter base)
    {
        ...
        File[] statisticsFiles = new File[nSubpop];
        statisticslog = new int[nSubpop];
        for(int subpop = 0; subpop < nSubpop; subpop++)
        {
            statisticsFiles[subpop] = state.parameters.getFile(base.push("subpop." + subpop + "." + P_STATISTICS_FILE),null);
            if (silentFile)
            {
                statisticslog[subpop] = Output.NO_LOGS;
            }
            else if (statisticsFiles[subpop]!=null)
            {
                try
                {
                    statisticslog[subpop] = state.output.addLog(statisticsFiles[subpop], !compress, compress);
                }
                catch (IOException i)
                {
                    state.output.fatal("An IOException occurred while trying to create the log " + statisticsFiles[subpop]+ ":\n" + i);
                }
            }
    ...
    public void postEvaluationStatistics(final EvolutionState state)
    {
        ...
        for(int x=0;x<state.population.subpops.length;x++)
        {
            ...
            if (doGeneration)
                state.output.println("\nGeneration: " + state.generation,statisticslog[x]);
            if (doGeneration) state.output.println("Best Individual:",statisticslog[x]);

            if (doGeneration) state.output.println("Subpopulation " + x + ":",statisticslog[x]);
            if (doGeneration) best_i[x].printIndividualForHumans(state,statisticslog[x]);
            ...
            if (doGeneration && doPerGenerationDescription)
            {
                if (state.evaluator.p_problem instanceof SimpleProblemForm)
                    ((SimpleProblemForm)(state.evaluator.p_problem.clone())).describe(state, best_i[x], x, 0, statisticslog[x]);
            }
        }
    }
    ...
    public void finalStatistics(final EvolutionState state, final int result)
    {
        ...
        for(int x=0;x<state.population.subpops.length;x++ )
        {
            if (doFinal) state.output.println("\nBest Individual of Run:",statisticslog[x]);
            if (doFinal) state.output.println("Subpopulation " + x + ":",statisticslog[x]);
            if (doFinal) best_of_run[x].printIndividualForHumans(state,statisticslog[x]);
            ...
            if (doFinal && doDescription)
                if (state.evaluator.p_problem instanceof SimpleProblemForm)
                    ((SimpleProblemForm)(state.evaluator.p_problem.clone())).describe(state, best_of_run[x], x, 0, statisticslog[x]);
        }
    }
}
