...
//		this.dataset = dataset;
	 ...
	 */
	public void buildIndex(int numberOfHashes, int numberOfHashTables, MersenneTwisterFast rand){
		...
		index = new Index(hashFamily, numberOfHashes, numberOfHashTables, rand);
//		if(dataset != null){
...
//		}
	...
	{
		if(index == null)
			throw new RuntimeException("Index is not built");
...
//	}
	...
	public static List<Vector> linearSearch(List<Vector> dataset,final Vector query,int resultSize,DistanceMeasure measure){
		...
		PriorityQueue<Vector> pq = new PriorityQueue<>(dataset.size(),dc);
		pq.addAll(dataset);
