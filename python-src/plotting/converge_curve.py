...
output_folder = Path('/home/mazhar/MyPhD/MyPapers/SubtreeContrib')
generations = 50
...
filter = ['analyze_terminals', 'gen35-49', 'fitall', 'terminal-prevgen',
          ...
          'FullTree75',
          'FullTree100',
          'GTLKnow', 'BestGen', 'fulltree', 'abscontrib',
          'TLGPCriptor',
          'FrequentSub-root',
          'subcontrib-gen49-49-all',
          'subcontrib-gen49-49-subtree',
          'frequent-Root'
          #    'subtree', 'fulltree', 'GTLKnowlege', 'BestGen', 'FrequentSub',
          ]
    ...
    def plot_mean_of_genbest_tr(gen_mean):
        ...
        for algorithm in sort_algorithms(gen_mean):
            ...
            if not 'writeknow' in algorithm:
                ...
                if 'wok' in algorithm:
                    ax_all.plot(range(1, len(mean_data) + 1), mean_data, linewidth=15, markersize=15,
                                label="Without Transfer", color='k')
                else:
                    sc += 1
                    sc %= 3
        ...
        fig_all.savefig(output_folder / experiment_path.name / f'{experiment_path.name}-all.jpg', bbox_inches='tight',
                        pad_inches=0)
...
def get_experiment_stats(experiment_path):
    ...
    def save_exp_stats():
        ...
        for algorithm in sorted(experiment_ts_fitnesses):
            ...
            miin_index = int(
                [i for i in experiment_ts_fitnesses[algorithm] if experiment_ts_fitnesses[algorithm][i] == miin][0])
            worst = max(experiment_ts_fitnesses[algorithm].values())
            worst_index = int(
                [i for i in experiment_ts_fitnesses[algorithm] if experiment_ts_fitnesses[algorithm][i] == worst][0])
            ...
            if algorithm != wok_exp:
                ...
                if len(experiment_ts_fitnesses[algorithm].values()) != 30 or len(
                        experiment_ts_fitnesses[wok_exp].values()) != 30:
                    print()
                pval = stats.wilcoxon(list(experiment_ts_fitnesses[algorithm].values()),
                                      list(experiment_ts_fitnesses[wok_exp].values()))[1]
            ...
            latex_file.write(
                f'{algorithm} & {miin} ({miin_index}) & {worst} ({worst_index}) & {mean} & {stdev} & {pval}\\\\\n')
